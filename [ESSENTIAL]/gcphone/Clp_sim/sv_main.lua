		...
		MySQL.Async.fetchAll(
			...
			function(result)
				...
				MySQL.Async.execute(
					...
					function (rowsChanged)
					TriggerClientEvent('esx:showNotification', source, "~g~Vous venez d'enregistrer une nouvelle carte sim.")
					end
				)
		end)
end

ESX.RegisterServerCallback('clp_sim:CheckSim', function(source, cb)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('clp_sim:sim_give')
AddEventHandler('clp_sim:sim_give', function (store, index)
  local _source = source
  local id = index
  local xPlayer = ESX.GetPlayerFromId(_source)
  ...
  TriggerClientEvent('esx:showNotification', _source, "Vous venez de donner la carte sim avec le numéro suivant : ~g~" .. simZ)
  TriggerClientEvent('esx:showNotification', id, "~g~Quelqu'un~s~ vous a donné une carte sim avec le numéro suivant : ~g~" .. simZ)
...
RegisterServerEvent('clp_sim:sim_delete')
AddEventHandler('clp_sim:sim_delete', function (sim)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local sim = sim
...
RegisterServerEvent('clp_sim:sim_use')
AddEventHandler('clp_sim:sim_use', function (sim)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	--local new = sim
...
ESX.RegisterServerCallback('clp_sim:GetList', function(source, cb)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local cartesim = {}
...
ESX.RegisterServerCallback('clp_sim:GetNameZ', function(source, cb, id)
