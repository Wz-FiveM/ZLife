TriggerEvent('es:addGroupCommand', 'tp', 'admin', function(source, args, user)
	...
	if x and y and z then
		...
		TriggerClientEvent('chatMessage', source, "SYSTEM", {255, 0, 0}, "Mauvaise coordonnées")
	end
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Mauvaises permissions.' } })
...
TriggerEvent('es:addGroupCommand', 'setjob', 'jobmaster', function(source, args, user)
	if tonumber(args[1]) and args[2] and tonumber(args[3]) then
		...
		if xPlayer then
			...
			TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Aucun joueur trouvé.' } })
		end
	else
		TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Mauvais usage.' } })
	end
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Mauvaises permissions.' } })
...
TriggerEvent('es:addGroupCommand', 'giveitem', 'admin', function(source, args, user)
	...
	if count ~= nil then
		if xPlayer.getInventoryItem(item) ~= nil then
			...
			TriggerClientEvent('esx:showNotification', source, "Vous avez envoyé ~g~"..count.." "..ESX.Items[item].label.."~s~ à l'id ~g~"..args[1].."~s~.")
		else
...
TriggerEvent('es:addGroupCommand', 'refreshproperties', 'admin', function(source, args, user)
	TriggerServerEvent('c_property:relaodproperties')
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Mauvaises permissions.' } })
end, {help = _U('chat_clear_all')})
