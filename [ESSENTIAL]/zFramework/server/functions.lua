...
ESX.SavePlayer = function(xPlayer, cb)
	...
	for i=1, #xPlayer.inventory, 1 do
		if xPlayer.inventory[i].count >= 1 then
			print(xPlayer.inventory[i].name, xPlayer.inventory[i].count, xPlayer.inventory[i].name)
		end
...
ESX.SavePlayers = function(cb)
	...
	for i=1, #xPlayers, 1 do
		table.insert(asyncTasks, function(cb)
			...
			--TriggerClientEvent('esx:showNotification', xPlayer.source, "~w~Synchronisation \n~g~Personnage synchronisé.")
		end)
	end

	Async.parallelLimit(asyncTasks, 8, function(results)
		RconPrint('[Save] Tout les joueurs' .. "\n")
...
ESX.CreatePickup = function(type, name, count, label, player, model)
    local pickupId = (ESX.PickupId == 65635 and 0 or ESX.PickupId + 1)

    ESX.Pickups[pickupId] = {type  = type, name  = name, count = count}
    TriggerClientEvent('esx:pickup', -1, pickupId, label, player, model)
    ESX.PickupId = pickupId
end

ESX.CreatePickupCoords = function(type, name, count, label, player, model, coords)
    local pickupId = (ESX.PickupId == 65635 and 0 or ESX.PickupId + 1)

    ESX.Pickups[pickupId] = {type  = type, name  = name, count = count}
    TriggerClientEvent('esx:pickupcoords', -1, pickupId, label, player, model, coords)
    ESX.PickupId = pickupId
end
