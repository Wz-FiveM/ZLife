			...
			function (licenses)
				if type ~= nil then
					for i=1, #licenses, 1 do
						if type == 'driver' then
							if licenses[i].type == 'drive' or licenses[i].type == 'drive_bike' or licenses[i].type == 'drive_truck' or licenses[i].type == 'peche' or licenses[i].type == 'chimielicense' then
								show = true
							end
				...
				if show then
					...
					TriggerClientEvent('esx:showNotification', _source, "~r~Vous n'avez pas cette license.")
				end
			end)
		end
	end)
