...
RegisterServerEvent('AnnounceCarDealerOuvert')
AddEventHandler('AnnounceCarDealerOuvert', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local xPlayers	= ESX.GetPlayers()
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Concessionnaire', '', '~g~Concessionnaire ouvert.', 'CHAR_SIMEON', 8)
	end
end)

RegisterServerEvent('AnnounceCarDealerFerme')
AddEventHandler('AnnounceCarDealerFerme', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local xPlayers	= ESX.GetPlayers()
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Concessionnaire', '', '~r~Concessionnaire fermé.', 'CHAR_SIMEON', 8)
	end
end)

RegisterServerEvent('clp_cardealer:annoncecardealer')
AddEventHandler('clp_cardealer:annoncecardealer', function(result)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local xPlayers = ESX.GetPlayers()
    local text = result
    if xPlayer.getMoney() >= 500 then 
        ...
        for i = 1, #xPlayers, 1 do
            local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
            TriggerClientEvent('GM:ShowAboveRadarMessageIcon', xPlayers[i], "WEB_PREMIUMDELUXEMOTORSPORT",1,"Concessionnaire","~b~Publicité",text, 1)
        end
    else
