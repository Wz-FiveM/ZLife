...
RegisterCommand("fac", function(user, args)
    local type = args[1]
    if type == "solo" then 
...
AddEventHandler("cBill:GetBill", function(bill)
    ...
    TriggerEvent("esx:showNotification", "Facture: ~b~"..bill.title.."~s~.\nMontant: ~g~"..price.."$~s~.")
    TriggerEvent("esx:showNotification", "Accepter: ~b~E ~s~ou Refuser: ~r~X~s~.")
