...
function Dialogue:CreateCam()
	local Player = GetPlayerPed()
	selfBaseCam = CreateCam("DEFAULT_SCRIPTED_CAMERA", false)
	SetCamCoord(selfBaseCam, Player.Pos.x, Player.Pos.y, Player.Pos.z+60.0)
    SetCamRot(selfBaseCam, -90.0, 0.0, 0.0, true)
    SetCamFov(selfBaseCam,100.0)
    ShakeCam(selfBaseCam,"HAND_SHAKE",0.7)
    SetCamActive(selfBaseCam, true)
	RenderScriptCams(true, true, 750, true, 0)
end
...
function round(num, numDecimalPlaces)
	local mult = 15^(numDecimalPlaces or 0)
...
function PMenu:resetMenu()
	...
	self.Base = {
		...
		Title = zCore and zCore.user and zCore.user.name or "Menu",
...
function PMenu:OpenMenu(stringName, boolBack)
	...
	local max = math.max(15, math.min(intSelect))
	self.Pag = { max - 14, max, intSelect, self.Pag[3] or 1 } -- min, max, current, ancien menu
...
function PMenu:canUseButton(button)
	...
	if button.role then
		...
		return userRole and tableValueFind(GetLinkedRoles(userRole), button.role)
	end
...
function PMenu:ProcessControl()
	...
	if (boolUP or boolDOWN) and currentButtonsCount and self.Pag[3] then
		if boolDOWN and (self.Pag[3] < currentButtonsCount) or boolUP and (self.Pag[3] > 1) then
			local value = (currentButtons[self.Pag[3]+1] and currentButtons[self.Pag[3]+1].separator) and 2 or 1
			local value2 = (currentButtons[self.Pag[3]-1] and currentButtons[self.Pag[3]-1].separator) and -2 or -1
			-- local value3 = self.Pag[3]-1 = 1 and -1 or value2
			self.Pag[3] = self.Pag[3] + (boolDOWN and value or value2)
			if currentButtonsCount > 15 and (boolUP and (self.Pag[3] < self.Pag[1]) or (boolDOWN and (self.Pag[3] > self.Pag[2]))) then
				self.Pag[1] = self.Pag[1] + (boolDOWN and value or value2)
				self.Pag[2] = self.Pag[2] + (boolDOWN and value or value2)
			end
		else
			local value = (currentButtons[1] and currentButtons[1].separator) and 2 or 1
			local value2 = (currentButtons[currentButtonsCount] and currentButtons[currentButtonsCount].separator) and currentButtonsCount -1 or currentButtonsCount
			self.Pag = { boolUP and currentButtonsCount - 14 or 1, boolUP and currentButtonsCount or 15, boolDOWN and value or value2, self.Pag[4] or 1 }
			if currentButtonsCount > 15 and (boolUP and (self.Pag[3] > self.Pag[2]) or (boolDOWN and (self.Pag[3] < self.Pag[1]))) then
	...
	if (boolRIGHT or boolLEFT) and currentBtn then
		...
		if currentMenu.slidemax or currentBtn and currentBtn.slidemax or self.Events["onSlide"] or slide then
			...
			if changeTo and not changeTo.slidefilter or changeTo and not tableValueFind(changeTo.slidefilter, self.Pag[3]) then
	...
	if self.Pag[3] and currentButtonsCount and self.Pag[3] > currentButtonsCount then
		self.Pag = { 1, 15, 1, self.Pag[4] or 1 }
	end
end
...
function PMenu:drawMenuButton(button, intX, intY, boolSelected, intW, intH, intID) --6,48,200
	...
	local stringPrefix = (((button.r and (((zCore and (zCore.jobRank < button.r or zCore.copRank < button.r)) or (button.rfunc and not button.rfunc())) and "~r~" or "")) or "") .. (self.Events["setPrefix"] and self.Events["setPrefix"](button, self.Data) or "")) or ""
	if not button.separator then
		DrawText2(0, button.name or "", .275, intX - intW / 2 + .005, intY - intH / 2 + .0025, tableColor)
	else
		DrawText2(4, (button.sepColor or "") .. (button.separator or ""), .375, intX - string.len(button.separator)/600, intY - intH / 2 + .0005, tableColor, true, true)	
	end
	...
	local textBonus = (self.Events["setBonus"] and self.Events["setBonus"](button, self.Data.currentMenu, self)) or (button.amount and button.amount) or (button.price and "~b~" .. math.floor(button.price) .. "$")
	if textBonus and string.len(textBonus) > 0 then
		DrawText2(4, textBonus, .375, intX + (intW / 2) - .005 - add,  intY - intH / 2 + .00075, tableColor, true, 2)
	end
end
...
function CreateRoue(currentMenu, cam, ped)
	...
	TriggerEvent("zCore:toggleroue", menuData, cam, ped)
end
