...
window.addEventListener("message", function(event) {
    if (event.data.action == "display") {
        ...
        $.post("http://zLegal/GiveItem", JSON.stringify({
...
function closeInventory() {
    $.post("http://zLegal/NUIFocusOff", JSON.stringify({}));
}
...
$(document).ready(function() {
    ...
    $(document).mousedown(function(event) {
        ...
        if (itemInventory == "second") {
            $.post("http://zLegal/TakeFromChest", JSON.stringify({
                item: itemData,
    ...
    $(document).mousedown(function(event) {
        ...
        if (itemInventory == "main") {
            $.post("http://zLegal/PutIntoChest", JSON.stringify({
                item: itemData,
        ...
        drop: function(event, ui) {
            ...
            if (type === "Chest") {
                ...
                if (itemData.type === "item_weapon") {
                    ...
                    $.post("http://zLegal/TakeFromFast", JSON.stringify({
                        item: itemData
                    }));
                }
            }
        ...
        drop: function(event, ui) {
            ...
            if (type === "Chest" && itemInventory === "main") {
                ...
                if (itemData.type === "item_weapon") {
                    disableInventory(500);
                    Hotbar[itemData.slot - 1] = false;
                    $.post("http://zLegal/PutIntoFast", JSON.stringify({
                        item: itemData
                    }));
                }
            }
