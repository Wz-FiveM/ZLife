...
function UIMenuListItem.New(Text, Items, Index, Description,price)
    ...
    local _UIMenuListItem = {
        Base = UIMenuItem.New(Text or "", Description or ""),
        Items = Items,
        LeftArrow = Sprite.New("commonmenu", "arrowleft", 110, 105, 18, 18),
        RightArrow = Sprite.New("commonmenu", "arrowright", 280, 105, 18, 18),
        ItemText = UIResText.New("", 290, 104, 0.28, 255, 255, 255, 255, 0, "Right"),
        Label = {
            Text = UIResText.New(price, 0, 0, 0.28, 245, 245, 245, 255, 0, "Right"),
            MainColour = {R = 255, G = 255, B = 255, A = 255},
            HighlightColour = {R = 0, G = 0, B = 0, A = 255},
        },
...
function UIMenuListItem:Position(Y)
    if tonumber(Y) then
        self.LeftArrow:Position(300 + self.Base._Offset.X + self.Base.ParentMenu.WidthOffset, 152 + Y + self.Base._Offset.Y)
        self.RightArrow:Position(400 + self.Base._Offset.X + self.Base.ParentMenu.WidthOffset, 152 + Y + self.Base._Offset.Y)
        self.ItemText:Position(300 + self.Base._Offset.X + self.Base.ParentMenu.WidthOffset, 148.5 + Y + self.Base._Offset.Y)
        self.Base:Position(Y)
    end
end
...
function UIMenuListItem:Draw()
    ...
    local Offset = MeasureStringWidth(Text, 0, 0.28)
...
function UIMenuHorizontalOneLineGridPanel.New(LeftText, RightText)
    _UIMenuHorizontalOneLineGridPanel = {
        ...
        ParentItem = nil,
        Text = {
            Left = UIResText.New(LeftText or "Left", 0, 0, 0.28, 255, 255, 255, 255, 0, "Centre"),
            Right = UIResText.New(RightText or "Right", 0, 0, 0.28, 255, 255, 255, 255, 0, "Centre"),
        },
    }
...
function UIMenuStatisticsPanel:AddStatistics(Name)
    local Items = {
        Text = UIResText.New(Name or "", 0, 0, 0.28, 255, 255, 255, 255, 0, "Left"),
...
function UIMenuGridPanel.New(TopText, LeftText, RightText, BottomText)
    _UIMenuGridPanel = {
        ...
        ParentItem = nil,
        Text = {
            Top = UIResText.New(TopText or "Top", 0, 0, 0.28, 255, 255, 255, 255, 0, "Centre"),
            Left = UIResText.New(LeftText or "Left", 0, 0, 0.28, 255, 255, 255, 255, 0, "Centre"),
            Right = UIResText.New(RightText or "Right", 0, 0, 0.28, 255, 255, 255, 255, 0, "Centre"),
            Bottom = UIResText.New(BottomText or "Bottom", 0, 0, 0.28, 255, 255, 255, 255, 0, "Centre"),
        },
    }
...
_UIMenuColourPanel = {
    ...
    Text = UIResText.New(Title.." (1 sur "..#Colours..")" or "Title".." (1 sur "..#Colours..")", 0, 0, 0.28, 255, 255, 255, 255, 0, "Centre"),
    ParentItem = nil,
}
...
function UIMenuPercentagePanel.New(MinText, MaxText)
    ...
    _UIMenuPercentagePanel = {
        ...
        Text = {
            Min = UIResText.New(MinText or "0%", 0, 0, 0.28, 255, 255, 255, 255, 0, "Centre"),
            Max = UIResText.New("100%", 0, 0, 0.28, 255, 255, 255, 255, 0, "Centre"),
            Title = UIResText.New(MaxText or "Opacity", 0, 0, 0.28, 255, 255, 255, 255, 0, "Centre"),
        },
        Audio = {Slider = "CONTINUOUS_SLIDER", Library = "HUD_FRONTEND_DEFAULT_SOUNDSET", Id = nil},
        ParentItem = nil,
    }
...
function UIMenu.New(Title, Subtitle, X, Y, TxtDictionary, TxtName,mouse,R,G,B,A)
   ...
   -- print(mouse)
    ...
    if Txt ~= "no" then 
        _UIMenu = {
            Logo = Sprite.New(TxtDictionary, TxtName, 0 + X, 27 + Y, 431, 80,0,R,G,B,A),
            Banner = nil,
            Title = UIResText.New(Title, 215 + X, 20 + Y, 0.70, 255, 255, 255, 255, 1, 1),
            Subtitle = {ExtraY = 0},
            WidthOffset = 0,
            Position = {X = X, Y = Y},
    ...
    if Subtitle ~= "" and Subtitle ~= nil then
        _UIMenu.Subtitle.Rectangle = UIResRectangle.New(0 + _UIMenu.Position.X, 107 + _UIMenu.Position.Y, 431, 33, 0, 0, 0, 255)
        _UIMenu.Subtitle.Text = UIResText.New(Subtitle, 8 + _UIMenu.Position.X, 111 + _UIMenu.Position.Y, 0.28, 245, 245, 245, 255, 0)
        ...
        _UIMenu.PageCounter.Text = UIResText.New("eee", 425 + _UIMenu.Position.X, 111 + _UIMenu.Position.Y, 0.28, 245, 245, 245, 255, 0, "Right")
        --_UIMenu.Subtitle.ExtraY = 33
    end
    ...
    _UIMenu.Description.Text = UIResText.New("Description", _UIMenu.Position.X + 5, 125, 0.28)

    _UIMenu.Background = Sprite.New("commonmenu", "gradient_bgd", _UIMenu.Position.X, 144 + _UIMenu.Position.Y - 5 + _UIMenu.Subtitle.ExtraY, 290, 25)
...
function UIMenu:SetMenuWidthOffset(Offset)
    if tonumber(Offset) then
        ...
        if self.Subtitle.Rectangle ~= nil then
            self.Subtitle.Rectangle:Size(431 + self.WidthOffset + 100, 33)            
...
function UIMenu:CalculateItemHeight()
    local ItemOffset = 0 + self.Subtitle.ExtraY - 33
...
function UIMenu:RecalculateDescriptionPosition()
    ...
    self.Description.Bar:Position(self.Position.X, 103 + self.Position.Y + WindowHeight)
    self.Description.Rectangle:Position(self.Position.X, 102.2 + self.Position.Y + WindowHeight)
    self.Description.Text:Position(self.Position.X + 8, 108 + self.Position.Y + WindowHeight)
...
function UIMenu:AddItem(Item)
    ...
    if #Items == 0 then
        ...
        if Item() == "UIMenuItem" then
            ...
            Item:Position((#self.Items * 20) - 33 + self.Subtitle.ExtraY)
...
function UIMenu:MultilineFormat(str)
    if tostring(str) then
        ...
        for i = 1, #words do
            local offset = MeasureStringWidth(words[i], 0, 0.28)
...
--[[     if self.WidthOffset > 0 then
    ...
    if #self.Items ~= 0 and self.Items[self:CurrentSelection()]:Description() ~= "" then
        ...
        self.Description.Rectangle:Size(431 + self.WidthOffset, ((Linecount == 1) and 33 or ((Linecount + 1) * 22)))
    end
end
...
function UIProgressBarItem.New(Text, X, Y, Heading, R, G, B, A)
    ...
    local _UIProgressBarItem = {
        ...
        Text = UIResText.New(Text or "N/A", 0, 0, 0.28, 255, 255, 255, 255, 0, "Centre"),
