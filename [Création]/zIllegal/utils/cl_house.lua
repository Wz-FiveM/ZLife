...
local robbableHouses = {
  ...
  [553] = {x = -213.86, y = 6396.5, z = 33.09, info = 'Procopio Drive 18'},
  [554] = {x = -189.07, y = 6409.72, z = 32.3, info = 'Procopio Drive 19'},
  [555] = {x = -167.23, y = 6439.25, z = 31.92, info = 'Procopio Drive 20 / Apt 1'},
  [556] = {x = -160.3, y = 6432.18, z = 31.92, info = 'Procopio Drive 20 / Apt 2'},
  [557] = {x = -150.38, y = 6422.38, z = 31.92, info = 'Procopio Drive 20 / Apt 3'},
  [558] = {x = -150.38, y = 6416.99, z = 31.92, info = 'Procopio Drive 20 / Apt 4'},
  [559] = {x = -157.31, y = 6409.99, z = 31.92, info = 'Procopio Drive 20 / Apt 5'},
  [560] = {x = -105.49, y = 6528.7, z = 30.17, info = 'Procopio Drive 21'},
  [561] = {x = -44.43, y = 6582.55, z = 32.18, info = 'Procopio Drive 22'},
  [562] = {x = -27.44, y = 6597.89, z = 31.87, info = 'Procopio Drive 23'},
  [563] = {x = 1.36, y = 6613.18, z = 31.89, info = 'Procopio Drive 24'},
  [564] = {x = 31.22, y = 6596.67, z = 32.83, info = 'Paleto Blvd 1'},
  [565] = {x = 11.5, y = 6578.22, z = 33.08, info = 'Paleto Blvd 2'},
  [566] = {x = -14.72, y = 6557.54, z = 33.25, info = 'Paleto Blvd 3'},
  [567] = {x = -374.77, y = 6190.77, z = 31.73, info = 'Paleto Blvd 5'},
}
...
local ihKb={[0]="Relativement calme",[4]="Bruit de pas",[7]="Très bruyant"}

local function dx(vDnoL55)
  local xlAK,zr1y=math.floor(vDnoL55*10),""
  for Hs,jk in pairs(ihKb)do 
      if Hs<=xlAK then 
          zr1y=jk 
      end 
  end;
...
AddEventHandler('houseRobberies:attempt', function(lockpicks)
 ...
 if lockpicks > 0 and NuitIs() and not isRobbing then
  local playerCoords = GetEntityCoords(PlayerPedId(), true)
  for id,v in pairs(robbableHouses) do
   if GetDistanceBetweenCoords(playerCoords, v.x, v.y, v.z, true) <= 2.5 then
    ...
    if chance >= 0 and chance <= 65 then
        TriggerServerEvent("call:makeCall","police",coords,"Cambriolage en cours.")
    end
    TriggerEvent('lockpickAnimation')
    createProgressBar("Crochetage en cours",0,255,197,150, 12000)
    Citizen.Wait(12000)
     isLockpicking = false
     pedSpawned = false
 ...
 elseif lockpicks == 0 and not isRobbing then
  ESX.DrawMissionText("~r~Aucun Lockpick", 3500)
 elseif not NuitIs() and not isRobbing then
  ESX.ShowNotification("~r~Les voisins sont réveillés\nRevenez ce soir.")
 else
 end
end)
...
AddEventHandler('houseRobberies:createHouse', function(id)
  PrepareMusicEvent("FINA_KILL_RESTART")
  TriggerMusicEvent("FINA_KILL_RESTART")
 ...
 while isRobbing do
  ...
  if IsPedSprinting(GetPlayerPed(-1)) then 
    ...
    ShowAboveRadarMessage("~r~Cambirolage annulé\n~s~Vous vous êtes fait repéré.")
  end
  if GetDistanceBetweenCoords(playerCoords, house.x+3.6, house.y-15, house.z-50, true) < 2.5 then
   ESX.ShowHelpNotification("Appuyez sur ~INPUT_PICKUP~ pour ~r~sortir")
...
function randomAI(generator)
 local modelhash = GetHashKey("ig_dale")
  ...
  if isRobbing then
   ...
   for i=1,#myRobbableItems do
    if (GetDistanceBetweenCoords(generator.x + myRobbableItems[i]["x"], generator.y + myRobbableItems[i]["y"], generator.z + myRobbableItems[i]["z"], GetEntityCoords(GetPlayerPed(-1))) < 1.4) and not myRobbableItems[i]['isSearched'] then
      DrawTopNotification("Appuyez sur ~INPUT_CONTEXT~ pour ~b~fouiller.")
     ...
     if IsControlJustReleased(1, 38) then
      ...
      createProgressBar("Fouille en cours",0,255,197,150, 20000)
      Citizen.Wait(20000)
...
function startAnim(lib, anim)
	ESX.Streaming.RequestAnimDict(lib, function()
		TaskPlayAnim(PlayerPedId(), lib, anim, 8.0, -8.0, 20000, 0, 0, false, false, false)
	end)
end
