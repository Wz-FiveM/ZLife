...
RegisterServerEvent('esx_health:update')
AddEventHandler('esx_health:update', function(health)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent("c_health:upstarhe")
AddEventHandler("c_health:upstarhe", function(xPlayer)
    local _source = source
    local xPlayer  = ESX.GetPlayerFromId(_source)
    ...
    if (result) then
        if xPlayer ~= nil then
            TriggerClientEvent("c_health:upstarhe", _source, json.decode(result[1].health))
        end
    end
end)
